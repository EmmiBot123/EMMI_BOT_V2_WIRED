const { shell } = window.require('electron'); // If using contextIsolation: false
	...
	if (window.profile[new_card].cpu != window.profile[card].cpu) {
		if (window.confirm(MSG['arduino_card'] + window.profile[new_card].description + ' ?')) {
			...
			if (new_prog != "python") {
				...
				else if (window.profile[new_card].cpu == "mrtnodesensor-esp32")   // ✅ new line
                    new_toolbox = "toolbox_arduino_all-mrtnodesensor";
				else if (window.profile[new_card].cpu == "mrtnodev1-esp32")   // ✅ new line
                    new_toolbox = "toolbox_arduino_all-mrtnodev1";
				else if (window.profile[new_card].cpu == "mrtnodeflipper-esp32")   // ✅ new line
                    new_toolbox = "toolbox_arduino_all-mrtnodeflipper";
...
BlocklyDuino.bindFunctions = function () {
	...
	$('#redirectBtn').on("click", function () {
		fetch('https://raw.githubusercontent.com/EmmiBot123/Ads/main/ad_link.txt')
    ...
    /* open/close level dropdown */
$('#btn_level_main').on("click", function (e) {
	e.stopPropagation();
...
$('#levelDropdown .dropdown-item').on("click", function (e) {
	e.preventDefault();
