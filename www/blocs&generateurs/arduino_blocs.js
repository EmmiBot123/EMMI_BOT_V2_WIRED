...
Blockly.Blocks["inout_digital_writev1"]={init:function(){
    var card=window.localStorage.card;
    this.setColour("#00929f");
    this.setHelpUrl(Blockly.Msg.HELPURL);
    this.appendDummyInput().appendField(Blockly.Msg.ARDUINO_INOUT_DIGITAL_WRITE_INPUT1).appendField(new Blockly.FieldDropdown(profile[card].dropdownDigital), "PIN");
...
Blockly.Blocks["inout_analog_read3"]={init:function(){
		var card=window.localStorage.card;
		var prog = window.localStorage.prog;
        this.setColour("#00929f");
        this.setHelpUrl(Blockly.Msg.HELPURL);
		if (prog != "python") {
			this.appendDummyInput().appendField(Blockly.Msg.ARDUINO_INOUT_ANALOG_READ_INPUT3).appendField(new Blockly.FieldDropdown(profile[card].dropdownAnalog), "broche");
        } else {
			this.appendDummyInput().appendField(Blockly.Msg.ARDUINO_INOUT_ANALOG_READ_INPUT3+" A0");
		}
		this.setOutput(true, "Number");
        this.setTooltip(Blockly.Msg.ARDUINO_INOUT_ANALOG_READ_TOOLTIP)}
};
Blockly.Blocks["read_analog_temp"] = {
  init: function () {
    var card = window.localStorage.card;
    var prog = window.localStorage.prog;
    ...
    if (prog != "python") {
      ...
      this.appendDummyInput()
        .appendField("read analog temp A0");
    }

    this.setOutput(true, "Number");
    this.setTooltip("Read analog temperature sensor and return °C");
  }
};
