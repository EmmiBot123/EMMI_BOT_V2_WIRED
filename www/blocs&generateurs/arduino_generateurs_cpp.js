...
Blockly.Arduino["inout_attachInterrupt_esp32"]=function(block){
	var dropdown_pin=block.getFieldValue('PIN');
	var dropdown_mode=block.getFieldValue('mode');
	var funcName='interrupt_'+dropdown_pin;
	Blockly.Arduino.setups_['setup_Interrupt_'+dropdown_pin]='pinMode('+dropdown_pin+', INPUT_PULLUP);\n  attachInterrupt(digitalPinToInterrupt('+dropdown_pin+'),'+funcName+','+dropdown_mode+');';
	var branch=Blockly.Arduino.statementToCode(block, 'DO' );
	Blockly.Arduino.codeFunctions_[funcName] ='void IRAM_ATTR ' + funcName + '() {\n' + branch + '}';
	return "";
};
...
Blockly.Arduino['VOICE_play'] = function(block) {
  ...
  Blockly.Arduino.includes_['include_emmi_voice'] =
...
Blockly.Arduino["VOICE_stop"] = function(block) {
...
Blockly.Arduino['speaker_init'] = function(block) {
  ...
  // One-time setup code
  Blockly.Arduino.setups_['speaker_init_setup'] =
    'pinMode(26, OUTPUT);\n' +
    'dacWrite(26, 128);\n'; // silence speaker
...
Blockly.Arduino["inout_digital_writev1"]=function(block){
    var dropdown_pin=block.getFieldValue("PIN");
    var dropdown_stat=Blockly.Arduino.valueToCode(block, "STAT", Blockly.Arduino.ORDER_ATOMIC);
    Blockly.Arduino.setups_["setup_output_" + dropdown_pin]="pinMode(" + dropdown_pin + ", OUTPUT);";
    var code = "digitalWrite(" + dropdown_pin + ", " + 
           (dropdown_stat == "HIGH" ? "LOW" : "HIGH") + ");\n";
...
Blockly.Arduino["inout_analog_read3"] = function(block) {
    var pin = block.getFieldValue('broche');

    // Add necessary includes and object definitions
    Blockly.Arduino.includes_['include_OneWire'] = '#include <OneWire.h>';
    Blockly.Arduino.includes_['include_DallasTemperature'] = '#include <DallasTemperature.h>';
    Blockly.Arduino.definitions_['define_OneWire'] = 'OneWire oneWire(' + pin + ');';
    Blockly.Arduino.definitions_['define_sensors'] = 'DallasTemperature sensors(&oneWire);';
    Blockly.Arduino.setups_['setup_sensors_begin'] = 'sensors.begin();';

    // Temperature reading code
    var code = '(sensors.requestTemperatures(), sensors.getTempCByIndex(0))';

    return [code, Blockly.Arduino.ORDER_ATOMIC];
};
Blockly.Arduino["read_analog_temp"] = function(block) {
  var pin = block.getFieldValue('broche');

  Blockly.Arduino.definitions_['define_math'] = '#include <math.h>';

  Blockly.Arduino.definitions_['define_ntc_params'] =
...
float TEMP_OFFSET = 9.0;`;
  ...
  Blockly.Arduino.setups_['setup_adc'] =
`analogReadResolution(12);
analogSetAttenuation(ADC_11db);`;

  // ✅ FIXED: explicit float lambda
  var code =
`(
  []() -> float {
